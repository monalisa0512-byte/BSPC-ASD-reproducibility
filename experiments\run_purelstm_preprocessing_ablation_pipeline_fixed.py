...
RESULTS_FOLDER = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "results", "purelstm_preprocessing_ablation_pipeline_fixed"))
os.makedirs(RESULTS_FOLDER, exist_ok=True)

# 设置日志文件
LOG_FILE = os.path.join(RESULTS_FOLDER, "purelstm_preprocessing_ablation_pipeline_fixed_log.txt")
...
def run_single_ablation(config, full_df_train, full_df_test, pid_to_label):
    ...
    for i, test_pid in enumerate(all_pids):
        ...
        model = PureLSTM(input_dim=input_dim).to(DEVICE)
...
def run_all_ablations():
    all_results = []
    log_print("\n" + "="*60)
    log_print("🚀 开始 PureLSTM 固定窗口预处理消融实验（训练/验证/测试同配置）")
    log_print("="*60)
    ...
    for idx, config in enumerate(ABLATION_CONFIGS):
        ...
        if result is not None:
            ...
            temp_csv_path = os.path.join(RESULTS_FOLDER, "purelstm_preprocessing_ablation_pipeline_fixed_temp.csv")
    ...
    csv_path = os.path.join(RESULTS_FOLDER, "purelstm_preprocessing_ablation_pipeline_fixed.csv")
