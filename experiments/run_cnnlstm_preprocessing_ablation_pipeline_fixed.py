...
# 设置日志文件
LOG_FILE = os.path.join(RESULTS_FOLDER, "cnnlstm_preprocessing_ablation_pipeline_fixed_log.txt")
...
def run_all_ablations():
    all_results = []
    log_print("\n" + "="*60)
    log_print("🚀 开始 CNNLSTM 固定窗口预处理消融实验（训练/验证/测试同配置）")
    log_print("="*60)
    ...
    for idx, config in enumerate(ABLATION_CONFIGS):
        ...
        if result is not None:
            ...
            temp_csv_path = os.path.join(RESULTS_FOLDER, "cnnlstm_preprocessing_ablation_pipeline_fixed_temp.csv")
    ...
    csv_path = os.path.join(RESULTS_FOLDER, "cnnlstm_preprocessing_ablation_pipeline_fixed.csv")
