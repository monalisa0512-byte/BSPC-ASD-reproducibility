r"""
AttentionNet nested-window LOSO with per-candidate seed control.

This is a copy of run_attentionnet_loso.py kept as a separate control script.
It preserves the nested window-selection protocol, but resets RNG state before
each fold/candidate training run so candidate results are not affected by the
order in which earlier candidates were trained.
"""
import argparse
import os
...
OUTPUT_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "results", "attention_nested_window_seed_control")
)
...
def run_pipeline_with_attention(data_folder=DATA_FOLDER, output_dir=OUTPUT_DIR):
    ...
    for i, test_pid in enumerate(all_pids):
        ...
        for candidate_idx, (window_size_ms, stride_ms) in enumerate(WINDOW_CANDIDATES):
            candidate_seed = SEED + (i + 1) * 1000 + candidate_idx
            ...
            set_experiment_seed(candidate_seed)
        ...
        fold_records.append({
            ...
            "candidate_seed": selected_candidate["candidate_seed"],
        })
