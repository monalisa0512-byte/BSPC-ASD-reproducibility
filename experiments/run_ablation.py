...
RESULTS_FOLDER = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "results", "ablation"))
os.makedirs(RESULTS_FOLDER, exist_ok=True)

# 设置日志文件
LOG_FILE = os.path.join(RESULTS_FOLDER, "ablation_log.txt")
...
ABLATION_CONFIGS = [
    AblationConfig(name="Ultimate", use_j_stat=True, pre_ms=80, post_ms=160,
                   impute_method='pchip', use_mask_features=True, trial_intercept=True, window_intercept=True),
    AblationConfig(name="Ablation_No_J_Stat", use_j_stat=False, pre_ms=80, post_ms=160,
                   impute_method='pchip', use_mask_features=True, trial_intercept=True, window_intercept=True),
    AblationConfig(name="Ablation_No_Expand", use_j_stat=True, pre_ms=0, post_ms=0,
                   impute_method='pchip', use_mask_features=True, trial_intercept=True, window_intercept=True),
    AblationConfig(name="Ablation_Linear_Impute", use_j_stat=True, pre_ms=80, post_ms=160,
                   impute_method='linear', use_mask_features=True, trial_intercept=True, window_intercept=True),
    AblationConfig(name="Ablation_No_Mask", use_j_stat=True, pre_ms=80, post_ms=160,
                   impute_method='pchip', use_mask_features=False, trial_intercept=True, window_intercept=True),
    AblationConfig(name="Ablation_Only_Trial_Intercept", use_j_stat=True, pre_ms=80, post_ms=160,
                   impute_method='pchip', use_mask_features=True, trial_intercept=True, window_intercept=False),
    AblationConfig(name="Ablation_Only_Window_Intercept", use_j_stat=True, pre_ms=80, post_ms=160,
                   impute_method='pchip', use_mask_features=True, trial_intercept=False, window_intercept=True),
    AblationConfig(name="Ablation_No_Intercept", use_j_stat=True, pre_ms=80, post_ms=160,
...
def create_dataset_from_full_data(full_df, train_pids, test_pid, config, scaler=None):
    ...
    test_max_missing_rate = ULTIMATE_MAX_MISSING_RATE
    test_max_continuous_frames = ULTIMATE_MAX_CONTINUOUS_FRAMES
...
def run_single_ablation(config, full_df_train, full_df_test, pid_to_label):
    ...
    for i, test_pid in enumerate(all_pids):
        ...
        y_pred_all.append(1 if final_score > best_thresh else 0)
    if len(y_true_all) == 0:
...
def run_all_ablations():
    all_results = []
    log_print("\n" + "="*60)
    log_print("🚀 开始消融实验（测试集绝对锁定原则）")
    log_print("="*60)
    log_print(f"结果将保存到: {RESULTS_FOLDER}\n")
    ultimate_config = ABLATION_CONFIGS[0]
    log_print("📥 加载测试集数据（Ultimate标准，所有实验共享）...")
    full_df_test = load_all_data(DATA_FOLDER, ultimate_config, is_train_data=False)
    pid_to_label = {pid: (1 if str(group[Y_COL].iloc[0]).strip().upper() == "ASD" else 0) for pid, group in full_df_test.groupby(PID_COL)}
    all_pids = list(pid_to_label.keys())
    log_print(f"总被试: {len(all_pids)} 人\n")

    for idx, config in enumerate(ABLATION_CONFIGS):
        log_print(f"\n{'='*60}")
        log_print(f"[{idx+1}/{len(ABLATION_CONFIGS)}] 准备运行: {config.name}")
        log_print(f"{'='*60}")
        log_print(f"📥 加载训练集数据（按 {config.name} 配置处理）...")
        full_df_train = load_all_data(DATA_FOLDER, config, is_train_data=True)
        result = run_single_ablation(config, full_df_train, full_df_test, pid_to_label)
        if result is not None:
            all_results.append(result)
            # 每完成一个实验就更新汇总结果（双重保险）
            temp_df = pd.DataFrame(all_results)
            temp_csv_path = os.path.join(RESULTS_FOLDER, "ultimate_ablation_results_temp.csv")
    ...
    csv_path = os.path.join(RESULTS_FOLDER, "ultimate_ablation_results.csv")
    results_df.to_csv(csv_path, index=False, encoding='utf-8-sig')
    log_print(f"✅ 最终结果已保存: {csv_path}\n")
    log_print(results_df.to_string())

    baseline_acc = float(results_df[results_df['Experiment'] == 'Ultimate']['Accuracy'].values[0].replace('%', ''))
    baseline_f1 = float(results_df[results_df['Experiment'] == 'Ultimate']['F1_Score'].values[0].replace('%', ''))
    log_print("\n" + "="*60)
    log_print("📉 各消融配置相对Ultimate的性能下降")
    log_print("="*60)
    for _, row in results_df.iterrows():
        if row['Experiment'] == 'Ultimate': continue
