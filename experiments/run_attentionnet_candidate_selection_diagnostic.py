...
This script does not modify the main training scripts. It runs the nested
window protocol and records every candidate window's validation metrics and
outer-test prediction, rather than only the selected candidate.

Seed policy:
- Each outer fold uses fold_seed = SEED + fold_index.
- Every candidate in the same fold is trained after resetting to that same
  fold_seed, so candidate comparisons are not affected by earlier candidates
  consuming random state or by different candidate-specific seeds.
"""
import argparse
import os
import random
...
DEFAULT_OUTPUT_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "results", "attention_candidate_selection_diagnostic")
)
...
def summarize_candidate_results(df, output_dir):
    ...
    # Same tie-breaker as the nested training script.
    df = df.copy()
    ...
    core_summary = pd.DataFrame([
        {
            "metric": "selected_candidate_accuracy",
            "value": selected_acc,
        },
        {
            "metric": "selected_candidate_f1",
            "value": selected_f1,
        },
        {
            "metric": "oracle_candidate_correct_rate",
            "value": oracle_rate,
        },
        {
            "metric": "selection_regret_rate",
            "value": regret_rate,
        },
        {
            "metric": "fixed_1000_500_candidate_accuracy",
            "value": fixed_acc,
        },
        {
            "metric": "fixed_1000_500_candidate_f1",
            "value": fixed_f1,
        },
        {
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# AttentionNet candidate-level nested selection diagnostic\n\n")
...
def run(data_folder, output_dir):
    ...
    for i, test_pid in enumerate(all_pids):
        ...
        pd.DataFrame(records).to_csv(os.path.join(output_dir, "candidate_level_results_partial.csv"), index=False)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run AttentionNet candidate-level nested selection diagnostic.")
