...
- [ ] Run `python smoke_check.py` in a fresh environment.
- [ ] Test one short CPU smoke run and one full GPU entry point from repository-relative paths.
- [ ] Confirm that no raw participant data, credentials, absolute paths, private logs, manuscripts, or reviewer documents are present.
- [ ] Create a GitHub repository and push this directory only.
